...
class NerfstudioDataParserConfig(DataParserConfig):
    ...
    custom_point_cloud: Optional[Path] = None
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        if self.config.load_3D_points:
            if self.config.custom_point_cloud is not None:
                ...
                CONSOLE.print(
                    "[bold yellow]Warning: load_3D_points set to true but no point cloud found. Gaussian splatting methods will use random point cloud initialization."
                )
                ply_file_path = None
